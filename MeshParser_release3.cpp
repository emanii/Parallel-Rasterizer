...
void calculateBoundingBox() {
   for(int i = 0; i < Triangles.size(); i++) {
      ...
      Vector2 vect1 = convertPointTo2d(Vertices[vert1 - 1], 
            new Vector3(-0.2,1,0));
      Vector2 vect2 = convertPointTo2d(Vertices[vert2 - 1], 
            new Vector3(-0.2,1,0));
      Vector2 vect3 = convertPointTo2d(Vertices[vert3 - 1], 
            new Vector3(-0.2,1,0));
...
void calculateBarycentric(int xAlign, int yAlign) {
   for(int i = 0; i < Triangles.size(); i++) {
      ...
      //      Triangles[i]->bbx.left, Triangles[i]->bbx.width, 
      //      Triangles[i]->bbx.height);
      ...
      for(int j = Triangles[i]->bbx.left; 
          j <= Triangles[i]->bbx.left + Triangles[i]->bbx.width; 
          ++j) {
         for(int k = Triangles[i]->bbx.top; 
             k <= Triangles[i]->bbx.top + Triangles[i]->bbx.height; 
             ++k) {  
            float x = (float)j;
            float y = (float)k;
            ...
            if(alpha >= 0 && alpha <= 1 && beta >= 0 && beta <= 1 && gamma >= 0 
                  && gamma <= 1) {
               float depthP = alpha * Triangles[i]->tri2d.v1.z_depth + 
                              beta * Triangles[i]->tri2d.v2.z_depth + 
                              gamma * Triangles[i]->tri2d.v3.z_depth;
               // 1 is camera z position
               float distancefromeye = 1 - depthP;
               if(distancefromeye <= z_buffer[j][k]) {
                  Vector2 vt = convertPointTo2d(new Vector3(j, k, depthP), 
                                                new Vector3(0,0,0));
                  ...
                  float R = 
                        computeColor(*Vertices[v1], Vertices[v1]->R) * alpha + 
                        computeColor(*Vertices[v2], Vertices[v2]->R) * beta + 
                        computeColor(*Vertices[v3], Vertices[v3]->R) * gamma;

                  float G = 
                        computeColor(*Vertices[v1], Vertices[v1]->G) * alpha + 
                        computeColor(*Vertices[v2], Vertices[v2]->G) * beta + 
                        computeColor(*Vertices[v3], Vertices[v3]->G)  * gamma;

                  float B = 
                        computeColor(*Vertices[v1], Vertices[v1]->B) * alpha + 
                        computeColor(*Vertices[v2], Vertices[v2]->B) * beta + 
                        computeColor(*Vertices[v3], Vertices[v3]->B)  * gamma;
...
void Parse() {
   ...
   ReadFile(str);
   // once the file is parsed find out the maximum extent to center and scale 
   // mesh
